...
try:
    ...
    # Export equity curve if available
    if 'trade_log' in results and results['trade_log']:
        equity_csv = r'c:\Users\HP\Downloads\_\_Td0\equity_curve.csv'
        print(f'\nExporting equity curve to {equity_csv}...')
        try:
            equity = [10000]  # initial amount
            equity_ts = [0]
            ts = 0
            closed_trades = [t for t in results['trade_log'] if t.get('status') == 'closed' and t.get('pnl') is not None]
            ...
            print(f'✓ Equity curve exported ({len(equity)} data points)')
            print(df_equity.to_string(index=False))
        except Exception as ex:
            print(f'Could not export equity curve: {ex}')
